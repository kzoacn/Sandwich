...
#define bf_get_bit bf192_get_bit


#define sf_load bf96_load
#define sf_store bf96_store
#define sf_from_bit bf96_from_bit
#define sf_zero bf96_zero
#define sf_one bf96_one
#define sf_add bf96_add
#define sf_mul bf96_mul
#define sf_sum_poly bf96_sum_poly
#define sf_inv bf96_inv
#define sf_count bf96_count
#define sf_from_bf64 bf96_from_bf64
#define sf_modulus bf96_modulus
#define sf_flip bf96_flip
#define sf_get_bit bf96_get_bit
#define sf_and bf96_and
//binary field
//subfield
...
sf_t LinP_eval_192(sf_t in, const sf_t LinP[BITS]){
    sf_t res = sf_zero();
...
void LinP_eval_bitlevel_192(const bf_t in[BITS], const sf_t LinP[BITS], bf_t out[BITS]){
...
void init_sandwich_192(sandwich_192_param_t* para){
    ...
    for(int i=BITS*2-1;i>=0;i--){
        alpha = bf192_shift_left_1(alpha);
        if(alpha_192_s[i] == '1')
...
void feistel_192(const sf_t state[3],const sf_t consts[2],const sf_t P[BITS], sf_t out_state[3], sf_t *witness, sf_t mul_in[2]){
	out_state[0] = state[1];
...
void feistel_bitlevel_192(int id,bf_t state[3][BITS],const sf_t consts[2],const sf_t P[BITS], const bf_t witness[BITS], const bf_t delta, bf_t out_state[3][BITS],bf_t mul_in0[BITS],bf_t mul_in1[BITS]){
    memcpy(out_state[0],state[1],sizeof(out_state[0]));
    bf_t in0[BITS],in1[BITS];

    for(int i=0;i<BITS;i++){
        in0[i] = bf_add(state[0][i],bf_public_192(id,sf_get_bit(consts[0],i),delta));
        in1[i] = bf_add(state[1][i],bf_public_192(id,sf_get_bit(consts[0],i),delta));
    }
    ...
    LinP_eval_bitlevel_192(out,P,tmp);
    for(int i=0;i<BITS;i++){
...
void inverse_192(const sf_t state[3],const sf_t consts,const sf_t P[BITS],sf_t out_state[3], sf_t *witness, sf_t *mul_in){
    ...
    out_state[0] = state[0];
    out_state[1] = LinP_eval_192(out, P);
    out_state[2] = state[2];
}



void inverse_bitlevel_192(int id,bf_t state[3][BITS],const sf_t consts,const sf_t P[BITS], const bf_t witness[BITS], const bf_t delta, bf_t out_state[3][BITS],bf_t mul_in[BITS]){
    bf_t in[BITS];

    for(int i=0;i<BITS;i++)
        in[i] = bf_add(state[1][i],bf_public_192(id, sf_get_bit(consts,i),delta));
...
void mds_192(sandwich_192_param_t* para,const sf_t state[3], sf_t out_state[3]){
    for(int i=0;i<3;i++){
        out_state[i] = sf_zero();
        for(int j=0;j<3;j++){
            out_state[i] = sf_add(out_state[i],
                sf_mul(sf_from_bf64(matMDS[i][j]) , state[j])
            );
        }
    }
}

void mds_bitlevel_192(sandwich_192_param_t* para,bf_t state[3][BITS],bf_t out_state[3][BITS]){
    for(int i=0;i<3;i++){
        //out_state[i] = bf192_zero();
...
void sandwich_192(sandwich_192_param_t* para,sf_t k0, sf_t k1, sf_t out[2],sf_t witness[9],sf_t mul_inputs[13]){
    ...
    for(int i=0;i<topRound;i++){
        feistel_192(top_state[i],top_consts[i],top_P[i],top_state[i+1],&witness[w_count++],&mul_inputs[m_count]);
        m_count+=2;
    }
    
    inverse_192(top_state[topRound],inv_const,inv_P,inv_state,&witness[w_count++],&mul_inputs[m_count++]);
    mds_192(para,inv_state,bot_state[0]);

    for(int i=0;i<bottomRound;i++){
        feistel_192(bot_state[i],bot_consts[i],bot_P[i],bot_state[i+1],&witness[w_count++],&mul_inputs[m_count]);
        m_count+=2;
    }
    
    mds_192(para,bot_state[bottomRound],final_state);
...
void sandwich_bitlevel_192(sandwich_192_param_t* para,int id, const bf_t out[2][BITS],const bf_t witness[9][BITS],const bf_t delta,
    ...
    for(int i=0;i<topRound;i++){
        feistel_bitlevel_192(id,top_state[i],top_consts[i],top_P[i],witness[w_count++],delta,top_state[i+1],&mul_inputs[m_count][0],&mul_inputs[m_count+1][0]);
        m_count+=2;
    }

    inverse_bitlevel_192(id,top_state[topRound],inv_const,inv_P,witness[w_count++],delta,inv_state,&mul_inputs[m_count++][0]);



    mds_bitlevel_192(para,inv_state,bot_state[0]);

    for(int i=0;i<bottomRound;i++){
        feistel_bitlevel_192(id,bot_state[i],bot_consts[i],bot_P[i],witness[w_count++],delta,bot_state[i+1],&mul_inputs[m_count][0],&mul_inputs[m_count+1][0]);
        m_count+=2;
    }    


    mds_bitlevel_192(para,bot_state[bottomRound],final_state);
...
void bf96_to_bf192_bitlevel(sandwich_192_param_t* para,const bf_t in[BITS], bf_t out[BITS*2]){
    bf_t a = bf_one();
...
bf_t bf192_convert_combine(sandwich_192_param_t* para,const bf_t in[BITS]){
    ...
    for(int i=BITS*2-1;i>=0;i--){
        ...
        //res = res<<1 
        const uint64_t mask = bf192_bit_to_uint64_mask(res, BITS*2-1);
        res                 = bf192_shift_left_1(res);
        res.values[0] ^= (mask & bf192_modulus);
