...
#define bf_get_bit bf256_get_bit


#define sf_load bf128_load
#define sf_store bf128_store
#define sf_from_bit bf128_from_bit
#define sf_zero bf128_zero
#define sf_one bf128_one
#define sf_add bf128_add
#define sf_mul bf128_mul
#define sf_sum_poly bf128_sum_poly
#define sf_inv bf128_inv
#define sf_count bf128_count
#define sf_from_bf64 bf128_from_bf64
#define sf_modulus bf128_modulus
#define sf_flip bf128_flip
#define sf_get_bit bf128_get_bit
#define sf_and bf128_and
//binary field
//subfield
...
sf_t LinP_eval_256(sf_t in, const sf_t LinP[BITS]){
    sf_t res = sf_zero();
...
void LinP_eval_bitlevel_256(const bf_t in[BITS], const sf_t LinP[BITS], bf_t out[BITS]){
...
void feistel_256(const sf_t state[3],const sf_t consts[2],const sf_t P[BITS], sf_t out_state[3], sf_t *witness, sf_t mul_in[2]){
	out_state[0] = state[1];
...
void feistel_bitlevel_256(int id,bf_t state[3][BITS],const sf_t consts[2],const sf_t P[BITS], const bf_t witness[BITS], const bf_t delta, bf_t out_state[3][BITS],bf_t mul_in0[BITS],bf_t mul_in1[BITS]){
    memcpy(out_state[0],state[1],sizeof(out_state[0]));
    bf_t in0[BITS],in1[BITS];

    for(int i=0;i<BITS;i++){
        in0[i] = bf_add(state[0][i],bf_public_256(id,sf_get_bit(consts[0],i),delta));
        in1[i] = bf_add(state[1][i],bf_public_256(id,sf_get_bit(consts[0],i),delta));
    }
    ...
    LinP_eval_bitlevel_256(out,P,tmp);
    for(int i=0;i<BITS;i++){
...
void inverse_256(const sf_t state[3],const sf_t consts,const sf_t P[BITS],sf_t out_state[3], sf_t *witness, sf_t *mul_in){
    ...
    out_state[0] = state[0];
    out_state[1] = LinP_eval_256(out, P);
    out_state[2] = state[2];
}



void inverse_bitlevel_256(int id,bf_t state[3][BITS],const sf_t consts,const sf_t P[BITS], const bf_t witness[BITS], const bf_t delta, bf_t out_state[3][BITS],bf_t mul_in[BITS]){
    bf_t in[BITS];

    for(int i=0;i<BITS;i++)
        in[i] = bf_add(state[1][i],bf_public_256(id, sf_get_bit(consts,i),delta));
...
void mds_256(sandwich_256_param_t* para,const sf_t state[3], sf_t out_state[3]){
    for(int i=0;i<3;i++){
        out_state[i] = sf_zero();
        for(int j=0;j<3;j++){
...
void mds_bitlevel_256(sandwich_256_param_t* para,bf_t state[3][BITS],bf_t out_state[3][BITS]){
    for(int i=0;i<3;i++){
        //out_state[i] = bf256_zero();
...
void sandwich_256(sandwich_256_param_t* para,sf_t k0, sf_t k1, sf_t out[2],sf_t witness[9],sf_t mul_inputs[13]){
    ...
    for(int i=0;i<topRound;i++){
        feistel_256(top_state[i],top_consts[i],top_P[i],top_state[i+1],&witness[w_count++],&mul_inputs[m_count]);
        m_count+=2;
    }
    
    inverse_256(top_state[topRound],inv_const,inv_P,inv_state,&witness[w_count++],&mul_inputs[m_count++]);
    mds_256(para,inv_state,bot_state[0]);

    for(int i=0;i<bottomRound;i++){
        feistel_256(bot_state[i],bot_consts[i],bot_P[i],bot_state[i+1],&witness[w_count++],&mul_inputs[m_count]);
        m_count+=2;
    }
    
    mds_256(para,bot_state[bottomRound],final_state);
...
void sandwich_bitlevel_256(sandwich_256_param_t* para,int id, const bf_t out[2][BITS],const bf_t witness[9][BITS],const bf_t delta,
    ...
    for(int i=0;i<topRound;i++){
        feistel_bitlevel_256(id,top_state[i],top_consts[i],top_P[i],witness[w_count++],delta,top_state[i+1],&mul_inputs[m_count][0],&mul_inputs[m_count+1][0]);
        m_count+=2;
    }

    inverse_bitlevel_256(id,top_state[topRound],inv_const,inv_P,witness[w_count++],delta,inv_state,&mul_inputs[m_count++][0]);



    mds_bitlevel_256(para,inv_state,bot_state[0]);

    for(int i=0;i<bottomRound;i++){
        feistel_bitlevel_256(id,bot_state[i],bot_consts[i],bot_P[i],witness[w_count++],delta,bot_state[i+1],&mul_inputs[m_count][0],&mul_inputs[m_count+1][0]);
        m_count+=2;
    }    


    mds_bitlevel_256(para,bot_state[bottomRound],final_state);
...
void bf128_to_bf256_bitlevel(sandwich_256_param_t* para,const bf_t in[BITS], bf_t out[BITS*2]){
    bf_t a = bf_one();
